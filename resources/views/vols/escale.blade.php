    ...
    <div id="full-stars-example-two">
        <div class="rating-group">
            ...
            <label aria-label="1 star" class="rating__label" for="rating3-1">
                <a href="{{ route('vols.searchVol', ['sort'             => request('sort') , 
                                                            'search_vol_depart' => request('search_vol_depart'),
                                                            'search_vol_arrivee'    => request('search_vol_arrivee'),
...
@section('total-recherche')
    ...
    <div class="rt-widget widget_rating">
        @if($type_de_vol != "Vols en Escale")
        <h6>{{ $vols->total() }}  service(s) trouves </h6>
         @endif   
    </div>
...
<form action="{{route('vols.searchVol')}}">
    ...
    <input type="hidden" name="sort" value="{{request('sort')}}">
    @endif
    @if(request('search_vol_depart') != null)
    <input type="hidden" name="search_vol_depart" value="{{request('search_vol_depart')}}">
    @endif
    @if(request('search_vol_arrivee') != null)
    <input type="hidden" name="search_vol_arrivee" value="{{request('search_vol_arrivee')}}">
    @endif
    @if(request('date_depart') != null)
    <input type="hidden" name="date_depart" value="{{request('date_depart')}}">
    @endif
    @if(request('date_retour') != null)
    <input type="hidden" name="date_retour" value="{{request('date_retour')}}">
    @endif
...
$avion1_arrivee = $product->lieu_depart;
    ...
    <div class="flight-list-box rt-mb-30 row">
        ...
        <div class="col-md-2 px-0 row">
            ...
            <h4 class="text-center col-sm-12">Prix Total</h4>
                ...
                <p class="trip text-center col-sm-12" style="font-size: 20px;font-weight: 600;color: rgba(255, 167, 0, 0.79);float: right;3">
                    {{ getPriceHelper_Pourcentage($product->prix + $avion1->prix , $product->promotion_pourcentage + $avion1->promotion_pourcentage)}}
                </p>
                @endif

                <div class="col-sm-12">
                    <a href="{{ route('vols.showEscale', [ $product->slug , $avion1->id]) }}" class="btn btn-danger rt-gradient text-uppercase mx-auto col-sm-12">Voir</a>
                </div>
         ...
         <!----------DETAILS------------------>
        <div class="colflight-list-box rt-mb-30 pt-30 container mx-0 collapse" id="{{($product->id+33)}}" style="width: 100%;">
            <div style="display: flex;">
                ...
                <div class="col-lg-6">
                    ...
                    <ul class="flight-timeline" style="width: 100%;padding-left: 4%;">
                        <li class="d-block">
                            <span><?php echo date("M j,  H:i  ", strtotime( $avion1->date_depart )); ?></span>

                        </li>
                        <li class="d-block"><span>{{ $avion1->aeroport_depart}}</span></li>
                        <li class="d-block">
                            <span><?php 
                                    echo $jour_mois_arrivee_avion1;
                                    echo "  ";
                        ...
                        <li class="d-block"><span>{{ $avion1->aeroport_arrivee }}</span></li>
                    </ul>
                    <br>
                    <div>Retour</div>
                    <ul class="flight-timeline" style="width: 100%;padding-left: 4%;">
                        <li class="d-block">
                            <span><?php echo date("M j,  H:i  ", strtotime( $avion1->date_retour )); ?></span>
