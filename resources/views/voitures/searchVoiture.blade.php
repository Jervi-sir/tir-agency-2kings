    ...
    <div id="full-stars-example-two">
        <div class="rating-group">
            ...
            <label aria-label="1 star" class="rating__label" for="rating3-1">
                <a href="{{ route('voitures.searchVoiture', ['sort'             => request('sort') , 
                                                            'search_voiture_location' => request('search_voiture_location'),
                                                            'search_voiture'    => request('search_voiture'),
...
@section('total-recherche')
    ...
    <div class="rt-widget widget_rating">
        <h6>{{ $voitures->total() }}  service(s) trouvés </h6>
    </div>
...
<div class="container-flex row" style="padding: 0;">
    ...
    <a class="sort-size col-8" href="{{ route('voitures.searchVoiture', ['sort'     => 'asc' ,
                                                'search_voiture_location' => request('search_voiture_location'),
                                                'search_voiture' => request('search_voiture'),
                                                'date_debut' => request('date_debut'),
                                                'date_fin' => request('date_fin'),
    ...
    <a class="sort-size col-8" href="{{ route('voitures.searchVoiture', ['sort'     => 'desc' ,
                                                'search_voiture_location' => request('search_voiture_location'),
                                                'search_voiture' => request('search_voiture'),
                                                'date_debut' => request('date_debut'),
                                                'date_fin' => request('date_fin'),
...
<form action="{{route('voitures.searchVoiture')}}">
    ...
    <input type="hidden" name="sort" value="{{request('sort')}}">
    @endif
    @if(request('search_voiture_location') != null)
    <input type="hidden" name="search_voiture_location" value="{{request('search_voiture_location')}}">
    @endif
    @if(request('search_voiture') != null)
    <input type="hidden" name="search_voiture" value="{{request('search_voiture')}}">
    @endif
...
<div class="flight-list-box rt-mb-30">
    <div class="top-content d-flex flex-md-row flex-column justify-content-lg-between">
         <div class="car-thumb mr-4 mr-lg-0 mb-5 mb-md-0" style="max-width: 15em;">
                    <img src="{{ asset('storage/' . $product->image) }}" alt="image voiture" class="rt-border-primary2">
        ...
        <div class="price-mant  row" style="display: block;">
